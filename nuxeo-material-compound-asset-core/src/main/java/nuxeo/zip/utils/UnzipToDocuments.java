 ...
 * This class is copied from https://github.com/nuxeo-sandbox/nuxeo-zip-utils
 * I did not add it as a dependency since it's published in a private repo currently.
 * TODO: add dependency when `nuxeo-zip-utils` is published.
 *
 * @since 10.2
 */
public class UnzipToDocuments {
    ...
    public DocumentModel run() throws NuxeoException {
        ...
        try {
            ...
            // documents. We do this by getting a prefix. This while-loop will iterate only the first items, until it
            // reaches a "non-ignorable" entry. If it is a directory, all is good. Else, a prexif will have to be used.
            ...
            while (sortedEntries.hasNext()) {
                ...
                if (entry.isDirectory()) {
                    ...
                    folderishDoc = session.createDocumentModel(parentDocPath + "/" + path, dcTitle, type);
